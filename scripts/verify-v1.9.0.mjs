/**
 * ASTeam Agent - v1.8.3 Comprehensive Verification & Pre-flight Safety Suite
 * 
 * Verifies:
 ...
 * 7. Security Fence Pre-flight & Tri-State Elastic Bypass (v1.8.3)
 * 8. Enterprise Auto-Updater Engine & ASTeam Server Protocol Suite (v1.9.0)
 */
...
async function runTestSuite() {
  console.log('===========================================================');
  console.log('🚀 ASTeam Agent v1.9.0 Comprehensive Verification & Auto-Updater Suite');
  console.log('===========================================================\n');
...
`
  ...
  // -------------------------------------------------------------
  // Test Group 12: Security Fence Pre-flight & Tri-State Elastic Bypass (v1.8.3)
  // -------------------------------------------------------------
  console.log('\n🛡️ [Test Group 12] Security Fence Pre-flight & Tri-State Elastic Bypass (v1.8.3)');
  try {
    ...
    assert(!!securityFenceManager, 'securityFenceManager instance exists');
    assert(typeof securityFenceManager.sanitizeText === 'function', 'sanitizeText is exported');
    assert(typeof securityFenceManager.recordBypass === 'function', 'recordBypass is exported');

    // 12.1 Non-sensitive text 0-redaction pass-through
    const cleanPrompt = '请帮我使用 TypeScript 编写一个斐波那契数列函数，并编写对应的单元测试。';
    const cleanRes = securityFenceManager.sanitizeText(cleanPrompt);
    assert(cleanRes.redactedItems.length === 0, 'Clean prompt has 0 redacted items');
    assert(cleanRes.sanitized === cleanPrompt, 'Clean prompt text is untouched');
    assert(!cleanRes.isBlocked, 'Clean prompt is not blocked');

    // 12.2 Sensitive text multi-asset detection (Private IP, API Key, DB URI)
    const sensitivePrompt = `请检查以下配置：
1. 内部测试服务器 IP: 192.168.1.100 与 10.0.0.5
2. 第三方鉴权 Key: sk-ant-api03-abcdef1234567890abcdef1234567890abcdef
3. 数据库连接串: postgresql://postgres:MySecureP@ss123@192.168.1.50:5432/production_db
`;
    const sensitiveRes = securityFenceManager.sanitizeText(sensitivePrompt);
    assert(sensitiveRes.redactedItems.length >= 3, `Sensitive prompt accurately identified ${sensitiveRes.redactedItems.length} types of assets (>=3)`);
    assert(sensitiveRes.redactedItems.some(i => i.type.includes('192.168')), 'Detected 192.168.x private IP');
    assert(sensitiveRes.redactedItems.some(i => i.type.includes('Key') || i.type.includes('Token')), 'Detected API Key token');
    assert(sensitiveRes.redactedItems.some(i => i.type.includes('数据库')), 'Detected database connection credentials');
    assert(sensitiveRes.sanitized.includes('<PRIVATE_IP_REDACTED_'), 'Sanitized text replaced IP with protection placeholder');
    assert(!sensitiveRes.sanitized.includes('MySecureP@ss123'), 'Sanitized text stripped DB credentials');

    // 12.3 Record Bypass Audit Trail
    const prevLogs = securityFenceManager.getAuditLogs() || [];
    const prevCount = prevLogs.length;

    securityFenceManager.recordBypass(sensitiveRes.redactedItems);
    const updatedLogs = securityFenceManager.getAuditLogs() || [];
    assert(updatedLogs.length === prevCount + 1, 'Audit logs incremented by 1 after recordBypass');
    const latestLog = updatedLogs[updatedLogs.length - 1];
    assert(latestLog.bypassed === true, 'Latest audit log is flagged bypassed: true');
    assert(latestLog.blockReason.includes('主动确认豁免') || latestLog.blockReason.includes('Bypassed'), 'Audit log records user bypass rationale');
    assert(latestLog.redactedItems.length >= 3, 'Audit log accurately preserves bypassed sensitive item details');

    // 12.4 Harness-runner bypassSecurityFence execution path
    const harnessModule = loadEsbuildModule(path.join(rootDir, 'electron/harness-runner.ts'));
    assert(typeof harnessModule.runHarnessAgent === 'function', 'runHarnessAgent is exported');

    // 12.5 System Prompt Cleanliness: Ensure dummy IP 192.168.1.1 was eliminated
    const harnessSrc = fs.readFileSync(path.join(rootDir, 'electron/harness-runner.ts'), 'utf8');
    assert(!harnessSrc.includes('192.168.1.1'), 'harness-runner systemPrompt does not contain dummy 192.168.1.1 (zero self-tripping)');
    assert(harnessSrc.includes('bypassSecurityFence'), 'harness-runner respects bypassSecurityFence configuration flag');
    assert(harnessSrc.includes('onSystemNotice'), 'harness-runner isolates system notifications through dedicated channel');
  } catch (err) {
    assert(false, `Group 12 threw error: ${err.message}`);
  }

  // -------------------------------------------------------------
  // [Test Group 13] Enterprise Auto-Updater & ASTeam Server Protocol Suite (v1.9.0)
  // -------------------------------------------------------------
  console.log('\n🚀 [Test Group 13] Enterprise Auto-Updater & ASTeam Server Protocol Suite (v1.9.0)');
  const MOCK_SERVER_PORT = 4999;
  let mockServerInstance = null;

  try {
    // 13.1 Module & Singleton Verification
    const autoUpdaterModule = loadEsbuildModule(path.join(rootDir, 'electron/auto-updater.ts'));
    assert(autoUpdaterModule.AutoUpdaterManager !== undefined, 'AutoUpdaterManager class is exported');
    assert(autoUpdaterModule.autoUpdaterManager !== undefined, 'autoUpdaterManager singleton instance is exported');

    const updater = autoUpdaterModule.autoUpdaterManager;
    const initialStatus = updater.getStatus();
    assert(initialStatus.status === 'idle', 'Initial status is idle');
    assert(initialStatus.error === null, 'Initial error is null');

    // 13.2 Config Persistence & Retrieval
    const initialConfig = updater.getConfig();
    assert(typeof initialConfig.serverUrl === 'string', 'Config contains valid serverUrl');
    assert(typeof initialConfig.autoCheck === 'boolean', 'Config contains autoCheck boolean');

    const savedConfig = updater.saveConfig({ serverUrl: `http://127.0.0.1:${MOCK_SERVER_PORT}`, channel: 'stable' });
    assert(savedConfig.serverUrl === `http://127.0.0.1:${MOCK_SERVER_PORT}`, 'saveConfig updates serverUrl in memory and disk');

    // 13.3 Event Emitter Subscription
    let receivedEvents = [];
    const unsubscribe = updater.onEvent((payload) => {
      receivedEvents.push(payload);
    });
    assert(typeof unsubscribe === 'function', 'onEvent returns unsubscribe cleanup function');

    // 13.4 Spin Up Mock ASTeam Server for Protocol Testing
    const mockFixtureDir = path.join(testArtifactsDir, 'server_releases');
    if (!fs.existsSync(mockFixtureDir)) {
      fs.mkdirSync(mockFixtureDir, { recursive: true });
    }

    const testBinaryContent = 'ASTEAM_OFFICIAL_SETUP_BINARY_V190_' + 'B'.repeat(2048);
    const testInstallerName = 'ASTeam-Agent-Setup-1.9.0.exe';
    const testBinaryPath = path.join(mockFixtureDir, testInstallerName);
    fs.writeFileSync(testBinaryPath, testBinaryContent, 'utf-8');

    const realSha256 = crypto.createHash('sha256').update(testBinaryContent).digest('hex');
    ...
    mockServerInstance = http.createServer((req, res) => {
      ...
      if (pathname === '/api/v1/update/check') {
        ...
        res.end(JSON.stringify({
          code: 200,
          data: {
            hasUpdate: clientVer !== '1.9.0',
            currentVersion: clientVer || '1.8.3',
            latestVersion: '1.9.0',
            minSupportedVersion: '1.8.0',
            isMandatory: clientVer === '1.7.0',
            releaseDate: '2026-09-12T16:00:00Z',
            title: 'ASTeam Agent v1.9.0 自动更新演进版',
            changelog: ['【自动更新】私有云端静默检测与一键平滑重启'],
            assets: {
              installer: {
      ...
      if (pathname === `/api/v1/update/download/${testInstallerName}`) {
        res.writeHead(200, {
          ...
          'X-Checksum-SHA256': realSha256
        });
        res.end(testBinaryContent);
        return;
      }
    ...
    await new Promise((resolve) => mockServerInstance.listen(MOCK_SERVER_PORT, '127.0.0.1', resolve));
    assert(true, `Mock ASTeam Server successfully listening on port ${MOCK_SERVER_PORT}`);

    // 13.5 Check for Updates API Integration
    const updateResult = await updater.checkForUpdates(`http://127.0.0.1:${MOCK_SERVER_PORT}`);
    assert(updateResult !== null, 'checkForUpdates returned non-null update manifest');
    assert(updateResult.hasUpdate === true, 'hasUpdate is true for v1.8.3 -> v1.9.0');
    assert(updateResult.latestVersion === '1.9.0', 'latestVersion is v1.9.0');
    assert(updateResult.assets.installer.fileName === testInstallerName, 'Installer asset filename matches');
    assert(updater.getStatus().status === 'available', 'Updater state transitioned to available');

    // 13.6 Download & SHA-256 Validation (Pass Case)
    receivedEvents = [];
    const downloadRes = await updater.startDownload();
    assert(downloadRes.success === true, 'startDownload reported success');
    assert(fs.existsSync(downloadRes.filePath), 'Downloaded installer file physically exists on disk');
    assert(updater.getStatus().status === 'downloaded', 'Updater state transitioned to downloaded');
    assert(receivedEvents.some(e => e.type === 'progress' && e.progress.percent > 0), 'Emitted download progress events');

    // 13.7 SHA-256 Tamper Protection Gate
    tamperSha256 = true;
    await updater.checkForUpdates(`http://127.0.0.1:${MOCK_SERVER_PORT}`);
    const tamperedDownloadRes = await updater.startDownload();
    assert(tamperedDownloadRes.success === false, 'Tampered download was intercepted and rejected');
    assert(updater.getStatus().status === 'error', 'Updater state transitioned to error upon checksum mismatch');
    assert(updater.getStatus().error.includes('SHA-256 完整性校验失败'), 'Accurate error message highlights SHA-256 integrity failure');

    // 13.8 State Guard: prevent premature install
    ...
    assert(false, `Group 13 threw error: ${err.message}`);
  } finally {
