...
flowchart TD
  ...
  subgraph v180 ["v1.8.0 自主演进技能与原生 Office 生产力跃升 (已全面交付 · 当前基线)"]
...
## 📍 阶段七点一：v1.8.1 存储中枢全闭环与环境隔离优化（已全面交付 · 当前正式版）

### 目标与定位
彻底解决在智能体排查与解决复杂问题过程中对本地宿主 C 盘或外部其它 Agent 环境（如 Antigravity / Gemini / Claude）路径与默认配置的依赖与污染。实现配置信息、全局技能库、持久记忆、MCP 服务与交付制品的 **100% 自身封闭闭环**，Windows 系统智能优选非系统盘（默认 `D:\ASTeamData`）。

### 核心特性交付清单
1. **💾 Windows 非系统盘智能优选与配置自闭环**：
   - 启动自动探测非系统盘（首选 `D:\ASTeamData`），并在中枢根目录持久化一份自闭环 `storage-config.json`；
   - 彻底摆脱对 `C:\Users\<user>\.asteam` 隐藏目录的默认写入，支持随外挂数据盘即插即拔迁移；
2. **🔌 MCP 服务配置自闭环落盘与自愈加载**：
   - 扩展 MCP 配置文件持久化机制，自动在数据中枢根目录保存 `mcp_servers.json`；
   - 启动或打开设置时自动从存储中枢恢复 MCP 服务，客户端对齐升级至 v1.8.1；
3. **🎯 技能库纯净隔离加载与零外部泄漏**：
   - `SkillManager.loadGlobalSkills` 严格锁定 `storageHub.getSkillsDir()`，彻底切断对外部 `~/.asteam/skills` 的扫描，避免与其他 Agent 技能混淆污染；
4. **🧭 智能路径重定向与闭环提示词注入**：
   - `harness-runner` 深度注入【ASTeam 智能体专属存储中枢与环境自闭环】纪律与物理绝对路径；
   - 工具执行器底层将一切 `~/.asteam/...` 映射无缝重定向至 ASTeam 数据中枢，彻底杜绝路径幻觉与冲突；
5. **📦 零宿主依赖套件与安装包发布 (`release/`)**：
   - 纯 JS 企业级 PDF 生成与结构检视 (`generate_pdf`, `read_pdf` 基于 `pdf-lib` + `fontkit` CJK 子集化)；
   - Windows 文件独占锁碰撞韧性回退层 (`safeWriteFileSync` 遇锁自动另存 `_v2` / `_v3`)；
   - 纯 JS ZIP 打包与解压缩 (`compress_zip`, `extract_zip`)；
   - 宿主无 Git 环境优雅降级与 Windows 260 字符长路径免疫 (`\\?\` 前缀)；
   - 正式发布包生成完毕：`release/ASTeam Agent-Setup-1.8.1.exe` & `release/ASTeam Agent-Portable-1.8.1.exe`。

---

## 📍 阶段七点二：v1.8.2 内置统一 Skill & MCP 深度融合演化（✅ 已全面交付 · 当前正式版）

### 目标与定位
不考虑团队协作与企业级复杂功能，专注于**单机单智能体下的统一 Skill & MCP 机制的深度融合与极致体验**。实现技能两阶段渐进式激活（告别 Token 暴涨与指令干扰）、Windows `npx` 管道死锁防护、MCP 参数 Schema 本地预纠错、MCP Prompts 自动投影为 Skill、以及纯 JS 高频内置 MCP 工具扩展。

### 核心交付特性清单
1. **⚡ Windows `npx` 管道死锁免疫与 MCP 参数 Schema 预校验 (P0 · 已交付)**：
   - Windows 下命令为 `npx` / `npx.cmd` 时自动前置注入 `-y` 参数，彻底避免 Stdio 管道交互式询问导致的无响应假死；
   - 调用外部 MCP 工具前，自动基于工具 `inputSchema` 进行轻量级校验，缺失必填参数或字段类型不匹配时直接提供友好、精准的自纠错反馈；
2. **🧠 Skill 两阶段轻量索引与按需动态激活 (P1 · 已交付)**：
   - 告别全量技能 Prompt 静态堆叠导致的上下文膨胀（系统提示词缩减至极简 Micro Index，< 225 tokens，每次交互省下数千 Token）；
   - 支持动态两阶段激活：在输入 `@skill_id` 或意图触发命中时，以及调度器执行 `activate_skill` 工具时精准按需载入完整技能内容与规约；
3. **📜 标准化 YAML Frontmatter 扩展与 MCP 服务工具联动 (P2 · 已交付)**：
   - 自定义技能与自演进技能支持标准 YAML Frontmatter（解析与序列化支持 `name`, `version`, `triggers`, `recommendedTools`）；
   - 建立“规约 + 工具”的协同绑定（Tools Binding），激活技能时自动关联并提示所需内置或 MCP 工具；
4. **🔌 MCP Prompts 协议能力打通 (P3 · 已交付)**：
   - 完整打通 MCP 官方三要素（Tools、Resources、Prompts）；
   - 外部连接的 MCP 服务所暴露的 Prompts 模版一等公民投影为 ASTeam Agent 的可用 Skill（ID 前缀 `mcp_prompt:*`），实现生态一键互通；
5. **🛠️ 纯 JS 零配置高频本地 MCP 工具扩充与进程韧性 (P4 · 已交付)**：
   - 原生内置 `sqlite_query`（纯 JS/Node 内置数据库引擎，支持 SQL DDL/DML/SELECT 查询与结构分析）；
   - 原生内置 `code_ast_inspector`（免外部重型环境提取 JS/TS/Python/Go/Java 等源码符号大纲、类、函数、接口、依赖与圈复杂度）；
   - 原生内置 `port_process_manager`（本地网络端口占用诊断、PID 映射与进程过滤分析）；
   - 外部 Stdio 子进程生命周期异常监听（`error`/`exit`）与指数退避自动重连自愈机制；
6. **🧪 全量工程自动化验证套件 (121/121 项测试 100% 通过)**：
   - 脚本 `scripts/verify-v1.8.2.mjs` 覆盖 8 大测试组（121 项断言全部 PASS），`npm run build` 0 报错；
7. **📦 Windows 双端安装包正式打包就绪 (`release/`)**：
   - 独立标准安装包：`release/ASTeam Agent-Setup-1.8.2.exe` (121.2 MB)；
   - 单文件免安装便携版：`release/ASTeam Agent-Portable-1.8.2.exe` (120.9 MB)。

---

## 📍 阶段七点三：v1.8.3 出境安全前置交互预检与三态弹性放行（✅ 已全面交付 · 当前正式版）

### 目标与定位
彻底重构企业安全围栏交互模型，由传统“发出后硬拦截/盲目脱敏”转变为**出境前前置轻量交互预检（Security Pre-flight Modal）**，赋予开发者**脱敏后发送、原样发出（本次豁免）、返回修改**的三态弹性决策权，兼顾企业安全审计合规与日常联调排障灵活性；修复系统提示词与安全围栏的自锁假阳性问题，保障流式思考流与交付产物的纯净性。

### 核心交付特性清单
1. **🛡️ 输入框前置毫秒级安全预检 (P0 · 已交付)**：
   - 发送时在前端内存中毫秒级扫描待出境文本（含图片/文件附件元信息）；
   - 0 敏感特征时 0ms 零感知直通，发现敏感特征时自动弹出高颜值预检确认弹窗。
2. **⚡ 三态弹性放行与会话偏好记忆 (P0 · 已交付)**：
   - 支持「脱敏后发送 (推荐)」、「原样发出 (本次豁免)」、「返回修改」三态操作；
   - 支持「在当前会话中记住此选择」，会话内后续交互无感放行，新建会话自动恢复安全防护。
3. **📋 豁免审计不可篡改追踪 (P1 · 已交付)**：
   - 用户豁免放行时自动记录审计日志（`bypassed: true`、带脱敏指纹及豁免原因）；
   - 顶部导航「企业合规」看板全面联动。
4. **🧹 系统提示词与思考流物理隔离 (P2 · 已交付)**：
   - 移除系统提示词中 `192.168.1.1` 假阳性测试 IP，改为 `127.0.0.1`；
   - 系统级通知统一经由 `onSystemNotice` 渠道送入思考流，绝不污染模型的主回答与工具调用。
5. **🧪 全量工程自动化验证套件 (176/176 项测试 100% 通过)**：
   - 脚本 `scripts/verify-v1.8.3.mjs` 覆盖 12 大测试组（176 项断言全部 PASS），`npm run build` 0 报错。
6. **📦 Windows 双端安装包正式打包就绪 (`release/`)**：
   - 独立标准安装包：`release/ASTeam Agent-Setup-1.8.3.exe` (121.2 MB)；
   - 单文件免安装便携版：`release/ASTeam Agent-Portable-1.8.3.exe` (120.9 MB)。
...
| **v1.7.1** | 2026-09 | 工作台并排零遮挡分栏、Swarm 右侧常驻协同大屏、物理真实时钟注入、安全围栏报告聚合与全景多模态大屏联动 | ✅ 已全面落盘交付 |
| **v1.8.0** | 2026 Q4 | Hermes 式持久记忆与自演进技能、全量内置原生 Office 生产力套件、物理落盘门禁防幻觉、PowerShell 沙箱 | ✅ 已全面交付并验证 |
| **v1.8.1** | 2026 Q4 | 存储中枢全闭环架构、Windows D 盘智能优选、零宿主依赖套件 (PDF/ZIP/文件锁韧性/Git优雅降级)、安装包正式发布 | ✅ 已全面交付并验证 (安装包已就绪) |
| **v1.8.2** | 2026 Q4 | 内置统一 Skill & MCP 深度融合演化 (按需动态激活、npx管道死锁免疫、Schema预纠错、MCP Prompts投影为Skill、纯JS内置工具) | ✅ 已全面交付并验证 |
| **v1.8.3** | 2026 Q4 | 出境安全前置交互预检与三态弹性放行 (毫秒级特征预扫描、脱敏/豁免/返回三态决议、当前会话偏好记忆、审计留痕) | ✅ 已全面交付并验证 · 当前正式版 |
| **v1.9.0** | 2027 Q1 | 云端联动作战、分布式 Swarm 节点集群、团队协同作战流与 RBAC | 📅 远期规划 |
