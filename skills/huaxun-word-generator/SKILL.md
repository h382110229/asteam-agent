---
name: huaxun-word-generator
description: 按华讯（ECCOM）知识模板格式规范生成交付 Word（.docx）与 PDF。Use when user asks for 华讯模板/华讯交付文档/huaxun word/华讯格式文档/按华讯模板写方案、设计、验收报告、操作手册，or mentions 华讯知识模板格式规范. Grills for project/customer/logo metadata first, then builds structured content and outputs docx+pdf with fixed table alignment. Do NOT use for generic Word docs that are not Huaxun-branded.
---

# 华讯模板交付（huaxun-word-generator）v1.0

按华讯知识模板格式规范 v2.3，生成符合规范的 **Word + PDF** 交付文档。表格对齐已相对原模板修正（表头居中、数据左对齐、垂直居中）。

## Important

1. **先环境预检，再 grill，再生成**：缺依赖/模板/Word 时先给出安装部署说明，不要硬跑。
2. **保留原模板品牌层**：封面横幅/ECCOM logo/页眉客户 logo 位/页脚公司样式来自原模板，**禁止**清空页眉页脚图片。
3. **表格对齐必须走 `normalize_table_alignment`**。
4. PDF 优先 Word COM；不可用再用 `MIMO_SOFFICE`。不要 reportlab 手排。
