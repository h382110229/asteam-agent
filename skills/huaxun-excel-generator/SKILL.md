---
name: huaxun-excel-generator
description: 按华讯（ECCOM）Excel模板格式规范生成交付工作簿（.xlsx）。Use when user asks for 华讯Excel模板/华讯交付表格/huaxun excel/华讯格式表格/按华讯模板做通讯录、设备清单、IP规划、验收检查表、问题跟踪、进度计划, or mentions 华讯Excel模板（暂定）. Collects project metadata and sheet schema first, then builds 版本控制+关于+数据表 with fixed teal header style. Do NOT use for generic spreadsheets that are not Huaxun-branded.
---

# 华讯 Excel 交付（huaxun-excel-generator）v1.0.0

按华讯 Excel 模板（暂定）格式，生成符合规范的 **Excel（.xlsx）** 交付工作簿。  
固定保留「版本控制」「关于」品牌页，数据表统一青绿表头 + 微软雅黑 + 细边框；列宽按内容自适应。

## Important

1. **先环境预检，再收集元数据，再生成**：缺 openpyxl/模板时先说明，不要硬跑。
2. **保留模板结构**：「版本控制」「关于」必须存在且被填写；「关于」A1:J1 横幅**禁止删除**。
3. **表头样式统一**：`#FF008080` 底 + 白字加粗；短字段居中，长文本列左对齐。
...
- 「关于」A1:J1 横幅：文档标题 + `#FF008080` 白字加粗（禁止删除/取消合并）
- 数据表：R1 表名标题条（青绿底），R2 表头（白字加粗，居中）
