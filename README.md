...
### 7. 智能容错与高可用保障
- **“桌面”路径自动映射**：智能纠正大模型拟造的中文“桌面”路径（如 `C:/Users/桌面/...`），自动映射为 Windows 真实桌面绝对路径，且在 Project 模式下安全放行外部文档导出；
- **LLMAPI 网关动态路由与故障转移**：默认首选 `Auto` 网关动态路由调度最优模型，端侧具备无感故障转移容灾重试，确保长链路任务平滑完成。
