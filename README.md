...
- **v1.2.1（当前版本）**：交付物外置动作（默认浏览器打开/资源管理器定位）、工作台制品货架抽屉 (Artifacts Shelf)、慢思考提拔固化；
- **v1.3.0（核心底座）**：数据存储中枢（脱离 C 盘/自定义目录防冲突）、Memory 长期记忆体系（`.asteam/memory/`）、多 LLM 接入池容灾（524 自动重试）；
- **v1.4.0（工程进阶）**：`.asteamrules` 项目行为准则、全能 `@` 上下文补全（`@file` / `@git`）、影子快照时光机一键撤销；
- **v1.5.0（开放生态）**：官方标准 MCP (Model Context Protocol) 客户端集成、实时联网技术检索、状态栏 Token 压缩。
