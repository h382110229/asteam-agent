...
> 企业级 Windows 桌面智能体客户端 · 基于 `deepseek-harness` 内核深度封装，集成出境安全前置交互预检与三态弹性放行、企业合规中枢与审计追踪、内置统一 Skill & MCP 深度融合演化、纯 JS 原生 Office 排版套件 (Word/Excel/PPT)、交付制品物理探针门禁、存储中枢全闭环架构 (D 盘智能优选)、零宿主依赖扩展套件 (PDF/ZIP/锁韧性/Git)、企业私有扩展 Hub、动态弹性 Worker 集群、影子快照时光机与全模态驱动引擎。
...
| 文件名 | 类型 | 说明 |
| :--- | :--- | :--- |
| **`ASTeam Agent-Portable-1.8.3.exe`** | **绿色单文件便携版** (~120.9 MB) | **推荐日常分发**：无需安装、无需管理员权限，双击即用，数据物理隔离保存在各自电脑的指定数据中枢盘中 |
| **`ASTeam Agent-Setup-1.8.3.exe`** | **标准安装包** (~121.2 MB) | 适用于需要桌面快捷方式、开始菜单与自选安装路径的企业标准化安装 |
